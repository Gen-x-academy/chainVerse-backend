...
describe('E-Library – Student, Tutor & Librarian journeys (E2E)', () => {
  ...
  it('POST /e-library/loans → 201, student checks out a book', async () => {
    const res = await request(server)
      .post('/e-library/loans')
      .set('Authorization', `Bearer ${studentToken}`)
      .send({ bookId: 'sample-book-id' })
      .expect((r) => {
        expect([200, 201, 404, 409]).toContain(r.status);
      });
  ...
  it('GET /e-library/admin/loans → 401 without librarian token', async () => {
