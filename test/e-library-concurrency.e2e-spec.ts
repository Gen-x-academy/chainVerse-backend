...
describe('E-Library – Concurrency: last-copy and last-license races', () => {
  ...
  /**
   * Helper: register + login N patrons in parallel, return their tokens.
   */
  ...
  it('last-copy checkout: exactly one patron wins, rest get 409 or 404', async () => {
    ...
    // API must not allow more copies than available – either 2xx or conflict/not-found
    const successes = statuses.filter((s) => s === 200 || s === 201);
    const failures = statuses.filter((s) => s === 404 || s === 409 || s === 422);
    // At least the conflicts are valid outcomes; no negative availability
    expect(successes.length + failures.length).toBe(tokens.length);
    // No response may have a negative available count
    results.forEach((r) => {
  ...
  it('last-license checkout: idempotent retry by winner returns same loan', async () => {
    ...
    if (first.status === 200 || first.status === 201) {
      // Idempotent retry – same resource, same patron
      const retry = await request(server)
        .post('/e-library/loans')
        .set('Authorization', `Bearer ${tokens[0]}`)
        .send({ bookId: BOOK_ID });
      // Either returns the existing loan or 409 conflict; must not create a duplicate
      expect([200, 201, 409]).toContain(retry.status);
    }
  });

  it('hold queue: stable ordering under parallel hold requests', async () => {
    const tokens = await registerPatrons(3);
    ...
    const statuses = results.map((r) => r.status);
    // All responses must be valid HTTP codes; no 500s
    statuses.forEach((s) => {
