...
/**
 * Library health metrics, backup coverage, and reconciliation jobs.
 *
 * getHealthSnapshot()  – point-in-time counters for dashboards / alerts.
 * reconcile()          – detects invariant drift in loans, holds, and
 *                        licenses. Supports dry-run and bounded batch sizes
 *                        so it is safe to run in production.
 * getBackupCollections() – authoritative list of MongoDB collections that
 *                          must be included in every backup snapshot.
 *
 * Resolves #1074
 */
@Injectable()
export class LibraryHealthService {
  ...
  async getHealthSnapshot(): Promise<LibraryHealthSnapshot> {
    // In production, replace these with DB aggregate queries.
  ...
  /**
   * Reconciliation job for loans, holds, licenses, and copy-count balances.
   *
   * @param dryRun    When true, report drift without writing any fixes.
   * @param batchSize Maximum records to fix per run (default 100).
   */
  async reconcile(
    dryRun = true,
    batchSize = 100,
    ...
    // Replace stub zeros with real DB aggregate queries as domain models land.
    const loanDrifts = 0;
    const holdDrifts = 0;
    ...
    if (!dryRun && totalDrifts > 0) {
      ...
      this.logger.log(`Reconciliation fixed ${fixedItems}, skipped ${skippedItems}`);
    } else {
  ...
  /**
   * Returns the MongoDB collections that must be included in every backup.
   * Use from backup orchestration scripts or the ops runbook.
   */
