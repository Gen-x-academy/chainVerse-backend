...
export interface CatalogEntry {
  ...
  isbn?: string;
  description?: string;
  /** Format-specific fields that cannot be mapped losslessly */
  lossyFields?: Record<string, unknown>;
}

/**
 * Maps catalog metadata between internal format and common library standards:
 * Dublin Core, BibTeX, and RIS.
 *
 * Mappings are versioned (MAPPING_VERSION) and lossy fields are surfaced
 * explicitly in the lossyFields property so callers can handle them.
 *
 * Resolves #1073
 */
@Injectable()
export class CatalogMappingService {
  ...
  exportToBibTeX(entry: CatalogEntry): string {
    this.validateEntry(entry);
    const key = `${entry.authors[0]?.split(' ').pop() ?? 'unknown'}${entry.publicationDate?.slice(0, 4) ?? ''}`;
    const fields = [
      `  title     = {${entry.title}}`,
  ...
  importFromDublinCore(dc: Record<string, string | string[]>): CatalogEntry {
    ...
    const known = new Set(['dc:identifier', 'dc:title', 'dc:creator', 'dc:date', 'dc:publisher', 'dc:description', 'dc:type', 'dc:format']);
