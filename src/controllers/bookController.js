...
exports.getAllBooks = async (req, res) => {
  try {
    ...
    const books = await Book.find(query)
      .select(
        "title author description coverImage link isbn averageRating totalReviews ratingDistribution createdAt",
      )
      .limit(limit * 1)
      .skip((page - 1) * limit)
      .sort(sortOption);
  ...
  try {
    ...
    if (cachedData) {
      return handleSuccess(
        res,
        200,
        "Course books retrieved from cache",
        cachedData,
      );
    }
    ...
    const courseBooks = course.recommendedBooks
      ? course.recommendedBooks.map((item) => ({
          ...item.book,
    ...
    return handleSuccess(
      res,
      200,
      "Course books retrieved successfully",
      result,
    );
  } catch (error) {
